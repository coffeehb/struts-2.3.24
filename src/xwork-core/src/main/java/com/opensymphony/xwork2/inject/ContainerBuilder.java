...
/**
 * Builds a dependency injection {@link Container}. The combination of dependency type and name uniquely identifies a dependency mapping; you can use the same
 * name for two different types. Not safe for concurrent use.
 *
 * <p>
...
public final class ContainerBuilder {
     ...
     *
     * <p>
     * Sets scope to value from {@link Scoped} annotation on the implementation class. Defaults to {@link Scope#DEFAULT} if no annotation is found.
     *
     * @param type
     *            of dependency
     * @param name
    ...
    /**
     * Upon creation, the {@link Container} will inject static fields and methods into the given classes.
     *
     * @param types
    ...
    /**
     * Creates a {@link Container} instance. Injects static members for classes which were registered using {@link #injectStatics(Class...)}.
     *
     * @param loadSingletons
     *            If true, the container will load all singletons now. If false, the container will lazily load singletons. Eager loading is appropriate for
     *            production use while lazy loading can speed development.
    ...
    /**
     * Currently we only support creating one Container instance per builder. If we want to support creating more than one container per builder, we should move
     * to a "factory factory" model where we create a factory instance per Container. Right now, one factory instance would be shared across all the containers,
     * singletons synchronize on the container when lazy loading, etc.
     */
